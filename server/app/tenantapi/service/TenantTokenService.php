...
namespace app\tenantapi\service;
...
class TenantTokenService
{
    ...
    {
        $time = time();
        $adminSession = TenantAdminSession::where([['admin_id', '=', $adminId], ['terminal', '=', $terminal]])->find();
        //获取token延长过期的时间
        $expireTime = $time + Config::get('project.tenant_token.expire_duration');
        $adminTokenCache = new TenantAdminTokenCache();
        ...
        if ($adminSession) {
            ...
            $adminSession = TenantAdminSession::create([
                'admin_id' => $adminId,
                'terminal' => $terminal,
    ...
    {
        $time = time();
        $adminSession = TenantAdminSession::where('token', '=', $token)->findOrEmpty();
