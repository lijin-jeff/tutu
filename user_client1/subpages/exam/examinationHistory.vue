	...
	<view class="page tn-safe-area-inset-bottom">
		<tn-nav-bar fixed customBack :backgroundColor="mainColor">
			...
			<view class="tn-flex tn-flex-col-center tn-flex-row-center ">
				<text class="tn-text-bold tn-text-xl tn-color-white">考试历史</text>
			</view>
		</tn-nav-bar>
				...
				<block v-for="(it, index) in historyList" :key="index">
					<view class="tn-flex exam-li mt-10">
						<view class="txt tn-flex tn-flex-direction-column">
							<view class="fs-16 line2">{{it.examination.title}}</view>
							<view class="acea-row row-middle tn-padding-top">
								<view class="tn-color-gray">
									<text class="tn-icon-time"></text>
									<text class="ml-5">{{it.submit_time}}</text>
								</view>
								<view class="tn-margin-left-sm">
									<text class="tn-icon-success-circle"
										:style="{color: mainColor, paddingRight: '8rpx'}"></text>
									<text>{{it.correct_count}}</text>
									<text class="tn-icon-close-circle tn-padding-left-sm tn-color-red"
										:style="{paddingRight: '8rpx'}"></text>
							...
							<view class="acea-row row-middle tn-padding-top">
								<view class="tn-color-gray">
									...
									<text class="ml-5">{{it.paper_score}}</text>
								</view>
								<view class="tn-margin-left-sm">
									<text class="tn-icon-ticket"
										:style="{color: mainColor, paddingRight: '8rpx'}"></text>
									<text>{{it.basic_score}}</text>
									<text class="tn-icon-ticket tn-padding-left-sm tn-color-red"
										:style="{paddingRight: '8rpx'}"></text>
									<text>{{it.user_score}}</text>
								</view>
							</view>
						</view>
						<image :src="it.examination.image" class="pic"></image>
					</view>
	...
	export default {
		...
		methods: {
			...
			fetchData() {
				...
				this.$api.apiExaminationHistory(this.queryParams).then(res => {
					if (res.code === 1) {
