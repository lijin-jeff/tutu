	...
	<view class="container">
		<tn-nav-bar fixed customBack :backgroundColor="mainColor">
			...
			<view class="tn-flex tn-flex-col-center tn-flex-row-center">
				<text class="tn-text-bold tn-text-xl tn-color-white">{{formattedTime}}</text>
			</view>
		</tn-nav-bar>
		...
		<view class="">
			<tn-modal v-model="showSubmitResult" :title="showSubmitTitle" width="70%" @click="clickModalConfirm"
				:maskCloseable="false" :content="submitInfo.alter_msg" :button="button"></tn-modal>
		</view>
		<!-- 答题结果弹窗结束 -->
		...
		<view
			...
			<view class="tn-flex tn-flex-row-between tn-padding-left tn-padding-right">
				<view style="width: 70%;" class="tn-flex tn-flex-row-left">
					<view>
						<tn-button fontColor="#fff" :backgroundColor="mainColor" @click="examOptionChange(1)">上一题</tn-button>
					</view>
					<view class="tn-padding-left" >
						<tn-button fontColor="#fff" :backgroundColor="mainColor" @click="examOptionChange(2)">下一题</tn-button>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			clickModalConfirm(event) {
				...
				if (event.index === 1) {
					this.$func.tnRelunch('/subpages/exam/examinationQuestionLib')
				} else {
					this.$func.tnRelunch('/subpages/exam/examinationHistory')
				}
			},
			// 拉取考试基础信息
			fetchExamination() {
			...
			examOptionChange(type) {// 试题手动上下切换选项
				let index =  this.swiperCurrentIndex + 1
				if (type === 1) {
					index = this.swiperCurrentIndex - 1
				}
				this.swiperChange({
					detail: {
						current:index
			...
			selectOption(index, optionIndex, examType) {
				...
				if (this.questionList[index].option[optionIndex].is_selected) {
					this.questionList[index].option[optionIndex].is_selected = false
				} else {
				...
				this.questionList.forEach((opt, i) => {
					...
					if (i === index) {
						console.log('原始状态', opt.option[optionIndex].is_selected)
						if (opt.option[optionIndex].is_selected) {
							opt.option[optionIndex].status = 'correct'
						} else {
							opt.option[optionIndex].status = 'default'
						}
					}
			...
			fetchQuestionList() { // 拉取试题
				...
				this.$api.apiExaminationQuestionList({
					uid: this.questionUid,
				}).then(res => {
					uni.hideLoading()
					if (res.code === 1) {
