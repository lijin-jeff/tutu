					...
					<tn-list-cell :unlined="true" padding="26rpx 0rpx" :key="index">
						<view class="message">
							<view class="message__middle">
								...
								<view class="message__content tn-text-ellipsis">{{item.create_time}}</view>
							</view>
							<view class="message__right">
								<view class="message__time" :style="{color: mainColor}" v-if="item.action === 1">{{ `+` + item.change_amount}}</view>
								<view class="message__time" :style="{color: mainErrorColor}" v-else-if="item.action === 2">{{ `-` + item.change_amount}}</view>
								<view class="message__tips">
									<tn-tag :backgroundColor="parseInt(item.action_type) === 1 ? mainErrorColor : mainColor" fontColor="tn-color-white" shape="circle" width="auto"
										size="sm">{{ parseInt(item.action_type) === 1 ? '平台' : '用户' }}</tn-tag>
								</view>
							</view>
						</view>
					</tn-list-cell>
	...
	export default {
		...
		methods: {
			...
			fetchUserScore() {
				...
				this.searchWhere.action = this.currentTab
				this.$api.apiUserScoreList(this.searchWhere).then(res => {
					uni.hideLoading()
					if (res.code === 1) {
						this.scoreList.push(...res.data.lists)
						this.searchWhere.page_no += 1
						this.score = res.data.extend.user_score
						return
					}
					this.$func.showToast(res.msg)
				})
