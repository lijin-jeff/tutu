<template>
	<view class="page tn-safe-area-inset-bottom">
		...
		<view :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<view class="search-box" :style="{top: vuex_custom_bar_height + 'px'}">
				...
				<view class="acea-row row-middle relative search-item">
					<input type="text" placeholder-class="tn-icon-search plaClass" placeholder="搜索资源" class="input"
						...
						<view class="txt tn-flex tn-flex-direction-column">
							...
							<view class="acea-row row-middle tn-color-gray">
								...
								<view class="tn-margin-left-sm">
									...
									<text class="ml-5">{{it.year + `年`}}</text>
								</view>
							</view>
						</view>
		...
		<!-- 选择器 -->
		<view class="">
			<tn-select v-model="showCategoryMenu" :confirmColor="mainColor" valueName="value"
	...
	export default {
		...
		methods: {
			fetchResourceCategory() {
				this.$api.apiResourceCategory().then(res => {
			...
			fetchResourceList() {
				this.$api.apiResourceList(this.queryParams).then(res => {
					...
					this.resourceList.push(...res.data.lists)
				})
			},
			// 分类确认事件
			confirm(e) {
