	...
	<view class="page tn-safe-area-inset-bottom">
		<tn-nav-bar fixed customBack :backgroundColor="mainColor">
			...
			<view class="tn-flex tn-flex-col-center tn-flex-row-center">
				<text class="tn-text-bold tn-text-xl tn-color-white">在线搜索</text>
			</view>
		</tn-nav-bar>
		<view :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<view class="search-box" :style="{top: vuex_custom_bar_height + 'px'}">
				...
				<view class="acea-row row-middle relative search-item">
					<input type="text" placeholder-class="tn-icon-search plaClass" placeholder="请输你需要的内容" class="input"
						v-model="queryParams.title" confirm-type="search" @confirm="search" />
					<view class="search-btn absolute" @click="search">搜索</view>
				</view>
			</view>
			<view class="exam-ul">
				<block v-for="(it, index) in searchDataList" :key="index">
					<view class="tn-flex exam-li mt-10" @click="tl(it)">
						<view class="txt tn-flex tn-flex-direction-column">
							...
							<view class="acea-row row-middle tn-color-gray">
								<view>
									...
									<text class="ml-5">{{it.data_type_title}}</text>
								</view>
								<view class="tn-margin-left-sm">
									<text class="tn-icon-time"></text>
									<text class="ml-5">{{it.publish_time}}</text>
								</view>
							</view>
						</view>
			...
			<!-- 资源列表为空开始 -->
			<view :style="{paddingTop: vuex_custom_bar_height * 2 + 'px'}" v-if="searchDataList.length === 0">
				<tn-empty mode="list" text="暂无数据"></tn-empty>
			</view>
			<!-- 资源列表为空结束 -->
		...
		<!-- 选择器 -->
		<view class="">
			<tn-select v-model="showCategoryMenu" :confirmColor="mainColor" valueName="value" labelName="label" :list="category" @confirm="categoryConfirm"></tn-select>
		</view>
	</view>
</template>
		...
		methods: {
			...
			tl(row) {
				if (row.data_type === 'article') {
					this.$func.navigatorTo('/subpages/news/content?uid=' + row.uid)
