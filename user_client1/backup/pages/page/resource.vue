	...
	<view class="page tn-safe-area-inset-bottom">
		<tn-nav-bar fixed customBack :backgroundColor="mainColor">
			<view class="tn-flex tn-flex-col-center tn-flex-row-center ">
				<text class="tn-text-bold tn-text-xl tn-color-white">电子资料</text>
			</view>
		</tn-nav-bar>
		<view :style="{paddingTop: vuex_custom_bar_height + 'px'}">
			...
			<view class="exam-ul">
				<block v-for="(it, index) in resourceList" :key="index">
					<view class="tn-flex exam-li mt-10" @click="tl('/subpages/resource/content?uid=' + it.uid)">
						<view class="txt tn-flex tn-flex-direction-column">
							...
							<view class="acea-row row-middle tn-color-gray">
								<view>
									...
									<text class="ml-5">{{it.category.title}}</text>
								</view>
								<view class="tn-margin-left-sm">
									<text class="tn-icon-time"></text>
									<text class="ml-5">{{it.publish_date}}</text>
								</view>
							</view>
						</view>
						<view class="" style="height: 100%;">
							<view style="height: 100%;">
								<image :src="it.cover" class="pic"></image>
							</view>
						</view>
					</view>
				</block>
			...
			<!-- 资源列表为空开始 -->
			<view :style="{paddingTop: vuex_custom_bar_height * 2 + 'px'}" v-if="resourceList.length === 0">
				<tn-empty mode="list" text="暂无资源"></tn-empty>
			</view>
			<!-- 资源列表为空结束 -->
		...
		<!-- 选择器 -->
		<view class="">
			<tn-select v-model="showCategoryMenu" :confirmColor="mainColor" valueName="uid" labelName="title"
				mode="multi-auto" :list="resourceCategory" @confirm="confirm"></tn-select>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				resourceList: [{
					uid: '',
					title: '最强推理AI模型Deepseek问世，杭州深度求索科技股票大涨，杭州深度求索科技股票大涨',
					cover: 'http://cdn.image.tutudati.com/uploads/images/20241004/2024100409415606b318141.jpg',
					category: {
						uid: '',
						title: '科技',
					},
		...
		methods: {
			...
			onSelected(res) {
				this.queryParams.free_state = res[0][0].value
				this.queryParams.source_type = res[1][0].value
				this.queryParams.year = res[2][0].value
				this.search()
			},
			// 重置查询条件
			resetQuery() {
			...
			fetchData() {
				...
				this.$api.apiResourceList(this.queryParams).then((res) => {
					...
					if (res.data.items.length === 0) {
						this.$func.showToast('暂无更多数据')
						return
					}
					this.resourceList.push(...res.data.items)
